...
Response.prototype.responseHandler = function(req, res, next) {
  ...
  if(isApiPath(req.path)) {
    if (!res || !res.locals || res.locals.data === undefined || res.locals.data === null) {
      ...
      formatResponse(res.locals.data, req.user, req.accessToken, req.sanitizeOptions || {}, (res.locals.sanitizeData !== false), function(err, responseObject) {
        if(err) {
          next(err);
        } else {
 ...
 */
function formatResponse(v, user, accessToken, options, isSanitize, cb) {
  // Create the response object.
  var response = {};

  // Attempt to sanitize the data before sending it to the client.
  if(isSanitize) {
    sanitize(v, user, accessToken, options, function(err, v) {
      if(err) {
        cb(err);
      } else {
        response.response = v;
...
var sanitize = function(v, user, accessToken, options, cb) {
  var tasks = [];
  ...
  if (type === "array") {
    for (var i = v.length - 1; i >= 0; --i) {
      if (v[i] && v[i]["sanitize"]) {
        tasks.push(createSanitizeMethod(v[i], user, accessToken, options));
      } else {
        return cb(undefined, v);
      }
    }
  } else if (type === "object" && v && v["sanitize"]) {
    tasks.push(createSanitizeMethod(v, user, accessToken, options));
  } else {
    return cb(undefined, v);
  }
...
var createSanitizeMethod = function(obj, user, accessToken, options) {
  return function(cb) {
    obj.sanitize(user, accessToken, options, cb);
  }
};
    ...
    if(flags.hasOwnProperty(key)) {
      ...
      switch(key) {
        ...
        default:
          ...
          log.debug("Unknown response flag: %s ", key);
          break;
      }
...
var createExpiredAccessTokenError = function(req, locale) {
  var err = new Error(req.i18n.t(locale ||'server.error.expiredAccessToken') || locale);
  err.status = 401;
  return err;
};
...
var sanitizeRenderData = function(req, res, next) {
  ...
  if(_.isArray(data)) {
    formatResponse(data, req.user, req.accessToken, req.sanitizeOptions || {}, (res.locals.sanitizeData !== false), function(err, responseObj) {
      if(err) {
        next(err);
      } else {
...
Response.prototype.createExpiredAccessTokenError = createExpiredAccessTokenError;
